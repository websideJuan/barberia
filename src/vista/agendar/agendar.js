    ...
    document.body
      ...
      .addEventListener("click", async (e) => {
        ...
        if (e.target.tagName === "P") {
          ...
          selectHorario.innerHTML = `
            <select name="horario" id="horario" class="horario_select">
              ${horasActual.map(
                (hora) =>
                  `<option class="${
                    hora.disponible ? "dia-active" : "dia-inactive"
                  }" value="${hora.hora}">${hora.hora} - ${
                    hora.disponible ? "Disponible" : "Ocupada"
                  }</option>`
              )}
            </select>
          `;
          ...
          document.body
            .querySelector('select[name="horario"]')
            .addEventListener("change", (e) => {
              const hora = e.target.value;
              calendario.value = `${dia}-${mes}/${hora} ${
                hora < 12 ? "AM" : "PM"
              }`;
    ...
    this.root.querySelector("form").addEventListener("submit", (e) => {
      ...
      if (cliente) {
        this.render(
          this.root,
          "p",
          `Cliente ${nombre} ${apellido} agregado con éxito.`
        );
      } else {
      ...
      document.body.querySelector(".popUp").addEventListener("click", (e) => {
        if (e.target.tagName === "BUTTON") {
          ...
          popUp.remove();
          this.barbero.setNotificaciones(
            `Nuevo cliente agendado para el dia ${
              diaAgendado.split("/")[0]
            } a las ${diaAgendado.split("/")[1]}`
          );
